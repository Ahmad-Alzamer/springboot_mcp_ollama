source URL: https://docs.spring.io/spring-ai/reference/api/etl-pipeline.html 
...
The Extract, Transform, and Load (ETL) framework serves as the backbone of data
processing within the Retrieval Augmented Generation (RAG) use case.

The ETL pipeline orchestrates the flow from raw data sources to a structured
vector store, ensuring data is in the optimal format for retrieval by the AI
model.

The RAG use case is text to augment the capabilities of generative models by
retrieving relevant information from a body of data to enhance the quality and
...
The `Document` class contains text, metadata and optionally additional media
types like images, audio and video.
...
The `Document` class content is created from PDFs, text files and other document
types with the help of `DocumentReader`.

To construct a simple ETL pipeline, you can chain together an instance of each
type.
...
To perform the basic loading of data into a Vector Database for use with the
Retrieval Augmented Generation pattern, use the following code in Java function
style syntax.
...
Alternatively, you can use method names that are more naturally expressive for
the domain
...
The `JsonReader` processes JSON documents, converting them into a list of
`Document` objects.
...
The `JsonReader` now supports retrieving specific parts of a JSON document using
JSON Pointers. This feature allows you to easily extract nested data from
...
This method allows you to use a JSON Pointer to retrieve a specific part of the
JSON document.
...
In this example, if the `JsonReader` is configured with `"description"` as the
`jsonKeysToUse`, it will create `Document` objects where the content is the
value of the "description" field for each bike in the array.
...
The `TextReader` processes plain text documents, converting them into a list of
`Document` objects.
...
The `JsoupDocumentReader` processes HTML documents, converting them into a list
of `Document` objects using the JSoup library.
...
The `JsoupDocumentReaderConfig` allows you to customize the behavior of the
`JsoupDocumentReader`:
...
The `JsoupDocumentReader` processes the HTML content and creates `Document`
...
The reader preserves the text content of the selected elements, but removes any
HTML tags within them.

###  Markdown

The `MarkdownDocumentReader` processes Markdown documents, converting them into
...
The `MarkdownDocumentReaderConfig` allows you to customize the behavior of the
...
Markdown also provides the possibility to `use inline code formatting
...
Behavior: The MarkdownDocumentReader processes the Markdown content and creates
Document objects based on the configuration:
...
the content of the Document objects.
...
The `ParagraphPdfDocumentReader` uses the PDF catalog (e.g. TOC) information to
split the input PDF into text paragraphs and output a single `Document` per
paragraph. NOTE: Not all PDF documents contain the PDF catalog.
...
The `TikaDocumentReader` uses Apache Tika to extract text from a variety of
document formats, such as PDF, DOC/DOCX, PPT/PPTX, and HTML. For a comprehensive
list of supported formats, refer to the Tika documentation.
...
The `TextSplitter` an abstract base class that helps divides documents to fit
the AI model’s context window.

###  TokenTextSplitter

The `TokenTextSplitter` is an implementation of `TextSplitter` that splits text
into chunks based on token count, using the CL100K_BASE encoding.
...
Document doc1 = new Document("This is a long piece of text that needs to be
split into smaller chunks for processing.",

        Map.of("source", "example.txt"));
Document doc2 = new Document("Another document with content that will be split
...
List<Document> splitDocuments = this.splitter.apply(List.of(this.doc1,
this.doc2));
...
The `KeywordMetadataEnricher` is a `DocumentTransformer` that uses a generative
AI model to extract keywords from document content and add them as metadata.
...
The keyword extraction prompt can be customized by modifying the
`KEYWORDS_TEMPLATE` constant in the class. The default template is:

```

\{context_str}. Give %s unique keywords for this document. Format as comma
separated. Keywords:
...
Where `{context_str}` is replaced with the document content, and `%s` is
replaced with the specified keyword count.
...
Document doc = new Document("This is a document about artificial intelligence
...
this.enrichedDoc.getMetadata().get("excerpt_keywords");
...
The `SummaryMetadataEnricher` is a `DocumentTransformer` that uses a generative
AI model to create summaries for documents and add them as metadata. It can
generate summaries for the current document, as well as adjacent documents
(previous and next).
...
The summary generation prompt can be customized by providing a custom
`summaryTemplate`. The default template is:
...
The `FileDocumentWriter` is a `DocumentWriter` implementation that writes the
content of a list of `Document` objects into a file.
...
When `withDocumentMarkers` is set to true, the writer includes markers for each
...
This will write all documents to "output.txt", including document markers, using
all available metadata, and appending to the file if it already exists.
...
Provides integration with various vector stores. See Vector DB Documentation for
a full listing.
